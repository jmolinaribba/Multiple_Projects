...
#Fixed route for CSV file and database
import os

BASE_DIR = os.path.dirname(os.path.abspath(__file__))

DB_PATH = os.path.join(BASE_DIR, "workouts.db")
CSV_PATH = os.path.join(BASE_DIR, "exercises.csv")
...
def load_dashboard(top, left, right):
    ...
    # Remove invalid rows
    df = df.dropna(subset=["weight", "sets", "reps"])
...
def show_kpis(frame, df):
    ...
    volume = (df["weight"] * df["sets"] * df["reps"]).sum()
...
def plot_volume(frame, df):

    for w in frame.winfo_children():
        w.destroy()

    volume = df.groupby("date").apply(
...
def plot_muscles(frame, df):

    for w in frame.winfo_children():
        w.destroy()

    muscles = df["muscle_group"].value_counts()
